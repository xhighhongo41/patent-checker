...
Patent Checker は、あなた自身のソフトウェアプロジェクトに関係しうる公開特許を探索し、見つけたことを日付入りの手順記録形式のレポートとして書き留めるためのツールです。同じプロジェクトに対して、構想の段階、開発の途中、公開の前、更新のたびに、繰り返し実行することを前提にしています。2 回目以降の実行は、新しく出たものだけを検索し、監視している特許を確認し直し、前回から何が変わったかを伝えます。2 つの部分から成ります。
...
プロジェクトが進んだとき、またはレポートに書かれた次回確認日が来たときに、もう一度頼みます。

> patent-checker スキルを使って、このプロジェクトの以前の探索の続きをして。
...
同じものを二度ダウンロードしません。請求項はキャッシュにずっと残り、権利状態は 1 週間、ファミリーは 1 か月で取り直されます。すべての結果に取得時刻が付くので、レポートは事実が「いつ時点のものか」を書けます。以前の版で探索したプロジェクトでは、Skill がまず直近のレポートを台帳に取り込むことを提案します。
